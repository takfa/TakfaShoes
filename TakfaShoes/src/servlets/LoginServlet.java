		...
		if(btnSubmit!=null){
			if(login!=null && password!=null){
				...
				//On regarde maintenant pour le cas de la connexion
					...
					try {
						if (userManager.findByLoginAdmin(login, password)){
							//si c'est le cas alors on avance vers la page de chat
							getServletContext().getRequestDispatcher("/BackOfficeServlet?id="+login).forward(request,response);
						}else {
							//Sinon, on affiche la page d'erreur
						    getServletContext().getRequestDispatcher("/pageErreurBack.html").forward(request,response);
						}
					} catch (SQLException e) {
						e.printStackTrace();
					}
